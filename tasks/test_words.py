...
if len(word_list) > 0:
	...
	print("\nOK. Let's see how many words you remember.")
	print("You will have to translate {} words into {}." .format(test_range, language))
	...
	while item_counter < test_range:
		for testitem in word_list:
			...
			tested = False
			while tested == False:
				typed_word = input("\nWhat is the meaning of '{}'? " .format(testitem["target"]))
				if typed_word == testitem["word"]:
					corrects += 1
					...
					if try_count < 2:
						...
						wrong_list.append(testitem["word"])
						wrongs += 1
						testitem["wrong_count"] = int(testitem["wrong_count"]) +1
						tested = True
						print("No, the solution is: '%s'." %(testitem["word"]))
	...
	# Print evaluation of test round
	print("\n-----")
	...
	if len(wrong_list) > 0:
		...
		for capitem in recap_list:
			cap_count = 1
			typed_word = input("What is the meaning of '{}'? " .format(capitem["target"]))
			if typed_word == capitem["word"]:
